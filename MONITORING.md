...
🚀 애플리케이션 모니터링
  ...
  └ 📈 StockOps 애플리케이션 메트릭  ← Prometheus (api ServiceMonitor)
```
...
- **메트릭 = api 전용, 추적 = api+ai**: api(Spring)는 actuator로 메트릭을 노출하지만 ai(FastAPI)는 `/metrics`가 없다. ai는 메트릭 대신 분산 추적(OTel)으로 관측하기로 역할을 분리했고, 따라서 ServiceMonitor는 api만 둔다.
- **summary 타입의 한계**: `http_server_requests_seconds`가 히스토그램 버킷 없는 summary라 진짜 p95는 불가하다. `_sum/_count`로 평균 응답시간만 산출한다. (정확한 분위수가 필요하면 앱에서 히스토그램 버킷을 켜야 함)
