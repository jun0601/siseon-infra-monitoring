...
prometheus:
  requests:
    memory: 768Mi    # 초기 256Mi → OOM으로 상향
    cpu: 100m
  limits:
    memory: 1.5Gi    # 초기 512Mi → OOMKilled 크래시 루프로 상향
    cpu: 500m
```

초기엔 t3.medium(2vCPU, 4GB)에 맞춰 보수적으로 512Mi로 설정했으나, WAF·멀티리전 로그/추적 누적 + 부하 트래픽으로 시계열 카디널리티가 늘며 OOMKilled가 반복돼 1.5Gi로 상향했다. request도 limit의 절반쯤(768Mi)으로 함께 올려 빠듯한 노드 재배치 시 또 OOM 나는 것을 방지. (트러블슈팅 #14)
...
🚀 애플리케이션 모니터링
  ...
  ├ 📜 StockOps 애플리케이션 로그    ← CloudWatch Logs (서울/오하이오 멀티리전)
  └ 📈 StockOps 애플리케이션 메트릭  ← Prometheus (api + 🤖 ai ServiceMonitor)

🛡️ 보안 모니터링
  └ 🛡️ StockOps WAF 보안 로그        ← CloudWatch Logs (서울 ALB/오하이오 ALB/CloudFront)
```

> IoT 센서/앱 로그는 서버 인프라가 아니라 **비즈니스·애플리케이션 데이터**이므로 인프라 폴더와 분리했다. WAF 로그는 주제가 보안이라 별도 `🛡️ 보안 모니터링` 폴더로 분리(보안 본체=진우 인프라, 시각화=관측 레이어로 역할 분리). WAF 리전 드롭다운은 긴 로그그룹명 대신 친근명(`서울 ALB`/`오하이오 ALB`/`CloudFront`)으로 노출하도록 커스텀 변수를 2개로 분리했다(트러블슈팅 #19). 같은 `folder` 값을 가진 provider는 하나의 폴더로 합쳐진다.

> **Grafana 접속**: ALB Ingress + ACM 와일드카드 인증서로 `https://grafana.siseon.live` HTTPS 노출(기존 NLB http에서 전환). Route 53 alias 레코드를 Terraform이 직접 관리한다.
...
### 📈 StockOps 애플리케이션 메트릭 (Prometheus / ServiceMonitor)

| 패널 | 타입 | PromQL 핵심 |
|------|------|------------|
| API 처리량 | timeseries | `rate(http_server_requests_seconds_count[1m])` (method별) |
| 에러율 | timeseries | 5xx / 전체 × 100 |
| 평균 응답시간 | timeseries | `rate(_sum) / rate(_count)` (uri별) |
| JVM 힙 메모리 | timeseries | `jvm_memory_used/committed_bytes{area="heap"}` |
| DB 커넥션 풀 | timeseries | `hikaricp_connections_active/idle/pending` |
| Bedrock 회로차단기 | stat | `resilience4j_circuitbreaker_state{state="open"}` |

**🤖 ai 패널 (FastAPI / ai-module)**

| 패널 | 타입 | 메트릭 핵심 |
|------|------|------------|
| 🤖 AI 예측 처리량 | timeseries | `rate(ai_forecast_requests_total[1m])` |
| 🤖 AI 예측 지연 p95 | timeseries | `ai_forecast_duration_seconds_bucket` (histogram → 진짜 p95) |
| 🤖 모델 캐시 적중률 | timeseries | `sum(ai_model_cache_events_total{result="hit"}) / sum(...) * 100` |
| 🤖 예측 정확도 MAPE | timeseries | `ai_evaluation_mape_percent` (평가 데이터 축적 시 표시) |

- **메트릭 = api + ai, 추적 = api + ai**: 초기엔 ai(FastAPI)에 `/metrics`가 없어 메트릭은 api 전용이고 ai는 분산 추적(OTel)으로만 관측했으나, 앱 업데이트로 ai-module에 `prometheus-fastapi-instrumentator`가 추가되며 `/metrics`가 생겨 ServiceMonitor를 부활시켰다. 이제 메트릭·추적 모두 api+ai 양쪽을 관측한다.
- **summary vs histogram**: api(`http_server_requests_seconds`)는 버킷 없는 summary라 진짜 p95 불가 → `_sum/_count` 평균만. 반면 ai 메트릭은 histogram이라 진짜 p95 산출 가능.
- **캐시 적중률 쿼리**: `increase([window])`는 idle 구간에 0/0=NaN이 되므로, 누적 hit/miss 비율(`sum(hit)/sum(total)`)로 계산해 트래픽 없을 때도 값이 유지되게 했다. (트러블슈팅 #16)
- **빈 패널 방지**: 에러율은 5xx가 없으면 No data가 뜨므로 `or vector(0)`로 0%를 표기. 회로차단기는 Bedrock 비활성 상태에서 closed(정상)로 표시.
- **공통 라벨**: 모든 PromQL에 `application="stockops"` 필터를 건다. `environment`는 배포 환경에 따라 값이 달라져(local 등) 필터에 넣지 않는다.

### ServiceMonitor 설계

별도 `servicemonitor.tf`에서 `kubernetes_manifest`로 ServiceMonitor를 정의한다.

```hcl
spec = {
  selector      = { matchLabels = { app = "stockops-api" } }
  namespaceSelector = { matchNames = ["stockops"] }
  endpoints = [{ targetPort = 8080, path = "/actuator/prometheus", interval = "30s" }]
}
# metadata.labels.release = "kube-prometheus-stack"  ← Prometheus가 인식하는 셀렉터
```

- ServiceMonitor가 대상 Service를 찾으려면 **Service 메타데이터에 `app` 라벨**이 있어야 한다(셀렉터 매칭). Deployment 라벨이 아니라 Service 라벨이 기준이다.
- ServiceMonitor는 **api + ai 둘 다** 둔다 — api는 `stockops-api-svc` `/actuator/prometheus`(8080), ai는 `stockops-ai-svc` `/metrics`(8000). 한때 ai에 `/metrics`가 없어 제거했다가 앱 업데이트로 부활시켰다.
- `kubernetes_manifest`는 plan 시점에 CRD가 존재해야 하므로, 클린 배포 시 kube-prometheus-stack(CRD 생성)을 먼저 apply한 뒤 ServiceMonitor를 apply하는 2단계가 필요하다. (트러블슈팅 #12)
...
dashboardProviders = {
  "dashboardproviders.yaml" = {
    providers = [
      ...
      { name = "appmetric-custom", folder = "🚀 애플리케이션 모니터링", ... }
    ]
  }
}
...
dashboards = {
  ...
  appmetric-custom = { stockops-appmetric = { json = jsonencode({ ... }) } }
}
```
...
## 🌐 Grafana 외부 노출 (ALB Ingress + HTTPS)

Grafana 외부 노출은 초기 **NLB(http)** 방식에서 **ALB Ingress(LBC) + ACM 와일드카드 인증서 HTTPS**로 전환했다. Grafana는 admin 로그인이 있어 http로 두면 비밀번호가 평문 노출되므로 도메인 + 인증서로 암호화했다.

```
외부 브라우저 → https://grafana.siseon.live
   → Route 53 (alias) → ALB(ACM 와일드카드 *.siseon.live)
   → ALB Ingress(LBC) → Grafana Pod(3000)
```

- **도메인**: `grafana.siseon.live` (Route 53 alias, Terraform이 직접 관리)
- **인증서**: ACM 와일드카드(`*.siseon.live`) — 진우 인프라의 ACM ARN을 `terraform_remote_state`로 참조해 재사용
- **헬스체크**: ALB TG 헬스체크 경로를 `/api/health`로 지정(기본 `/`는 Grafana가 302를 반환해 503 유발 → 트러블슈팅 #18)
...
> 초기 NLB 방식에서 사용하던 서브넷 태그·`internet-facing` annotation 구성은 LBC가 ALB/NLB 서브넷을 식별하는 공통 조건이라 ALB 전환 후에도 동일하게 필요하다.
