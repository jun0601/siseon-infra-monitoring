...
> IoT 센서/앱 로그는 서버 인프라가 아니라 **비즈니스·애플리케이션 데이터**이므로 인프라 폴더와 분리했다. WAF 로그는 주제가 보안이라 별도 `🛡️ 보안 모니터링` 폴더로 분리(보안 본체=진우 인프라, 시각화=관측 레이어로 역할 분리). 같은 `folder` 값을 가진 provider는 하나의 폴더로 합쳐진다.
